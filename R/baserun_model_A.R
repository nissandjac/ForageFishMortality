...
U <- matrix(0, 2 , data$iTimeMax)

parameters.tmb = list(Rmaxlog = log(100),
                  #SDlog = log(0.2),#,#,
                  SDlogsurv = log(0.01),
                  logSDM = log(0.2),#                    logSDR = log(0.5),
                  logSDF = log(1),
                  lognF = log(0.05),
                  logmuF = log(2),
                  lFstart = log(0.2),
...
compile("baserun_model_A.cpp")
dyn.load(dynlib("baserun_model_A"))

obj <-MakeADFun(data,parameters.tmb,DLL="baserun_model_A", random = 'U')
