...
# Extract parameters 
wInf <- parameters$wInf
Rmax <- parameters$Rmax
h <- parameters$h
a <- parameters$a
A <- parameters$A
kappa <- 1000 # Check if this matters
...
for (i in 2:nyear){
  errF[i] <- exp(rnorm(n = 1, mean = 0, sd = parameters$Fdev))
  Fforage[i] <- Fforage[i-1]*errF[i]

  Merr[i] <- rnorm(n = 1, mean = 0, sd = parameters$Mdev)
...
SF.year <- IterateSpectrum(param,F0,Merr)
...
for (i in 1:nyear){
    ...
    err <-  exp(rnorm(n = nsize-1,mean = 0, sd = parameters$surv.sd)) #  Error within that survey
    x <- tapply(SF.year$N[i,]*parameters$q*surv.sel, cuts, sum)
    x[is.na(x)] <- 0
...
err.catch <- exp(rnorm(n = nyear,mean = 0, sd = parameters$sd.catch))
