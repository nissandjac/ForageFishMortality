...
// Re-transform data and parameters from log
...
//for(int i=0;i<wlength;i++){ // Initialize vectors
//  // Fisheries selectivity
// Fsel(i) = Type(1.0)/(Type(1.0) + exp(-Type(muF)*(w(i)-nF*wInf)));
//
//}
//
//for(int i=0;i<wlength;i++){ // Initialize vectors
//  // survey selectivity
// surveysel(i) = Type(1.0)/(Type(1.0) + exp(-Type(muFs)*(w(i)-nFs*wInf)));
//
//}
...
for(int i=0;i<wlength;i++){ // Initialize vectors
  ...
  Fin(i,0) = Fsel(i)*exp(logF(0))*Fstart; // Initial fishing
...
Msavetot(0) = Mpred(widxM-1,0)+Zbase;
...
Msavetot(i) = Mpred(widxM-1,i)+Zbase;
}
...
//
// Calculate stuff
vector<Type> Ntemp(nlength);// for(int i=1;i<iTimeMax;i++){
//     ans+= -dnorm(logR(i),logR(i-1), SDR, TRUE); // M-Process likelihood
//  }
...
// for(int i=1;i<iTimeMax;i++){
//     ans+= -dnorm(Msave(i),Mpredin(widxM-1.0), Type(1), TRUE); // M-Process likelihood
//  }
